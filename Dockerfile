...
RUN export JOBBER_HOME=/tmp/jobber && \
    ...
    if  [ "${JOBBER_VERSION}" != "latest" ]; \
      then wget --directory-prefix=/tmp https://github.com/dshearer/jobber/releases/download/v1.1/jobber-1.1-r0.x86_64.apk && \
        apk add --allow-untrusted /tmp/jobber-1.1-r0.x86_64.apk ; \
      else mkdir -p $JOBBER_LIB && \
